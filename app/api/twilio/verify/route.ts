...
export async function GET() {
  ...
  const APP_SID = process.env.TWILIO_APP_SID || process.env.TWILIO_TWIML_APP_SID
  ...
  try {
    // Use the SAME creds you sign your token with
    const client = twilio(API_KEY_SID, API_KEY_SECRET, { accountSid: ACCOUNT_SID })

    // Try to fetch the TwiML App with those creds
    ...
    return NextResponse.json(
      { ok: false, message: e?.message, code: e?.code, status: e?.status, moreInfo: e?.moreInfo },
      { status: 500 }
    )
  }
}
